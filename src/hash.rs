...
fn calc_sig_zero(curr_row: &[char]) -> Vec<char> {
	let v1 = bin_util::bin_rot(curr_row, 7);
	let v2 = bin_util::bin_rot(curr_row, 18);
...
fn calc_sig_one(curr_row: &[char]) -> Vec<char> {
	let v1 = bin_util::bin_rot(curr_row, 17);
	let v2 = bin_util::bin_rot(curr_row, 19);
...
fn calc_final_hash(message_block: Vec<Vec<char>>) -> String {
	let initial_hashes: Vec<Vec<char>> = calc_initial_hashes();
	...
	let result_word_one   = bin_util::bin_add(&a, &initial_hashes[0]);
	let result_word_two   = bin_util::bin_add(&b, &initial_hashes[1]);
	let result_word_three = bin_util::bin_add(&c, &initial_hashes[2]);
	let result_word_four  = bin_util::bin_add(&d, &initial_hashes[3]);
	let result_word_five  = bin_util::bin_add(&e, &initial_hashes[4]);
	let result_word_six   = bin_util::bin_add(&f, &initial_hashes[5]);
	let result_word_seven = bin_util::bin_add(&g, &initial_hashes[6]);
	let result_word_eight = bin_util::bin_add(&h, &initial_hashes[7]);
...
fn calc_choice(e: &[char], f: &[char], g: &[char]) -> Vec<char> {
 	let not_e = bin_util::bin_not(&e);
 	let e_and_f = bin_util::bin_and(&e, &f);
 	let not_e_and_g = bin_util::bin_and(&not_e, &g);
 	bin_util::bin_xor(&e_and_f, &not_e_and_g)
}

fn calc_sum_one(input: &[char]) -> Vec<char> {
	let x = bin_util::bin_rot(&input, 6);
	let y = bin_util::bin_rot(&input, 11);
	let z = bin_util::bin_rot(&input, 25);
	let result = bin_util::bin_xor(&x, &y);
	bin_util::bin_xor(&result, &z)
}


fn calc_sum_zero(input: &[char]) -> Vec<char> {
	let x = bin_util::bin_rot(&input, 2);
	let y = bin_util::bin_rot(&input, 13);
	let z = bin_util::bin_rot(&input, 22);
	let result = bin_util::bin_xor(&x, &y);
	bin_util::bin_xor(&result, &z)
}

fn calc_majority(a: &[char], b: &[char], c: &[char]) -> Vec<char> {
	let x = bin_util::bin_and(&a, &b);
	let y = bin_util::bin_and(&a, &c);
	let z = bin_util::bin_and(&b, &c);
	let result = bin_util::bin_xor(&x, &y);
	bin_util::bin_xor(&result, &z)
}

fn calc_initial_hashes() -> Vec<Vec<char>> {
...
// TODO: not working when input > 55 characters
pub fn hash(input: String) -> String {
	let input_bin = bin_util::str_to_bin(input);
